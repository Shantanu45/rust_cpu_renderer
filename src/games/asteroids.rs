...
impl Ship{
    ...
    pub fn translate(&mut self, offset: Vec2i)
    {
        let x = self.model_mat.r1[0] + offset.x as f32;
        let y = self.model_mat.r2[1] + offset.y as f32;
        self.model_mat.r1[0] = x;
        self.model_mat.r2[1] = y;
    }
}
...
impl Asteroids{
    fn draw_ship(&self, renderer: &mut Renderer) {
        ...
        let ship_head = self.ship.model_mat * Vec3::from_vec2_1(SHIP_HEAD);
        let ship_right_wing = self.ship.model_mat * Vec3::from_vec2_1(SHIP_RIGHT_WING);
        let ship_left_wing = self.ship.model_mat * Vec3::from_vec2_1(SHIP_LEFT_WING);
        let ship_mid_section = self.ship.model_mat * Vec3::from_vec2_1(SHIP_MID_SECTION);
