...
impl<'a> Ui<'a> {
    ...
    pub fn menu_item(&mut self, rect: UiRect, text: &str, selected: bool) {
        ...
        self.renderer
            .draw_filled_quad(rect.top_left, rect.bottom_right, background);
        draw_text(
            self.renderer,
...
impl UiRect {
    pub const fn new(top_left: Vec2i, bottom_right: Vec2i) -> Self {
        Self {
            top_left,
            bottom_right,
        }
...
fn draw_glyph(renderer: &mut Renderer, pos: Vec2i, glyph: [u8; 7], color: Color, scale: i32) {
    for row in 0..GLYPH_HEIGHT {
        ...
        for col in 0..GLYPH_WIDTH {
            ...
            draw_scaled_pixel(renderer, pos.x + col * scale, pos.y + row * scale, color, scale);
        }
    }
}
